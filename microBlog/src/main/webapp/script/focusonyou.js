$(function(){
	$.ajax({
		url:"concern_getFansInfo.action",
		type:"POST",
		dataType:"Json",
		success:function(data){
			...
			htmlstr+='<td height="60" class="td3"><font color="#000000" size="3"><b>关注你的人</b></font></td>';
			...
			for (var i = 0; i < data.obj.length; i++) {
				if(data.obj[i].pic==null)
					{
					data.obj[i].pic='images/mainBannerContent2People1Img.gif';
					}
				htmlstr+=' <tr>';
			...
			htmlstr+='<tr>';
			htmlstr+=' <td height="65" align="left" class="font2" ><a href="MyWB.html" id="userLogin1"></a>';	
			htmlstr+=' </td>';
			htmlstr+='</tr>';
			htmlstr+='<tr>';
			...
			for (var i = 0; i < data.obj.length; i++) {
				htmlstr+='<img src="'+data.obj[i].pic+'" alt="" width="54" height="54" class="img" />';
				if(i%3==0 &&i!=0){
					htmlstr+='<br />'
				}
			}
			htmlstr+='<br />';
			for (var i = 0; i < data.obj.length; i++) {
				htmlstr+='&nbsp;&nbsp;&nbsp;&nbsp;<font color="#005dc3"> '+data.obj[i].nickname+'</font>';
			}
			htmlstr+='</td>';
			htmlstr+='</tr>';
